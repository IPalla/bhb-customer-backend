...
export class SquareServiceErrorFilter implements ExceptionFilter {
  private readonly logger = new Logger(SquareServiceErrorFilter.name);
    ...
    this.logger.error(`Square payment error on ${request.url}: ${JSON.stringify(errorInfo)}`);
    this.logger.debug(`Raw error details: ${JSON.stringify(exception.details, null, 2)}`);
